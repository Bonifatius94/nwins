...
{
    public class BitwiseGameStateTest
    {
        // initialize logging tools
        private readonly ITestOutputHelper _logger;
        public BitwiseGameStateTest(ITestOutputHelper logger) { _logger = logger; }
        ...
        public void ConstructorTest()
        {
            ...
            Assert.True(state.Fields.Length == 42);
            Assert.True(state.MaxRows == 6 && state.MaxColumns == 7);
            Assert.True(state.Fields.All(x => x == GameSide.None));

            // make sure that the constructor also works for non-standard sized boards
            state = GameStateFactory.CreateState(4, 5);
            Assert.True(state.Fields.Length == 20);
            Assert.True(state.MaxRows == 4 && state.MaxColumns == 5);
            Assert.True(state.Fields.All(x => x == GameSide.None));
        }
        ...
        public void ApplyActionTest()
        {
            ...
            try { state.ApplyAction(new GameAction(6, GameSide.SideA));   Assert.True(true); } catch (IndexOutOfRangeException) { Assert.True(false);  }
            try { state.ApplyAction(new GameAction(7, GameSide.SideA));   Assert.True(false);  } catch (IndexOutOfRangeException) { Assert.True(true); }
            ...
            var tempState = state;
            // fill the first column (this should not crash)
            try
            {
            ...
            // test if side A is forbidden to put a stone into the occupied column (this should crash)
            try
            {
            ...
            // test if side B is forbidden to put a stone into the occupied column (this should crash)
            try
            {
        ...
        public void GetPossibleActionsTest()
        {
            ...
            var initialState = GameStateFactory.CreateState(6, 7);
            var columnPermutations = SubSetsOf(Enumerable.Range(0, 7));
            ...
            foreach (var perm in columnPermutations)
            {
                ...
                foreach (int col in perm) { state = fillColumn(state, col); }
                bool allOccupied = perm.SequenceEqual(Enumerable.Range(0, 7));
                ...
                // test side A actions
                var expectedActions = Enumerable.Range(0, 7).Except(perm).Select(col => new GameAction((uint)col, GameSide.SideA));
                Assert.True(state.GetPossibleActions().SequenceEqual(expectedActions));

                // test side B actions
                if (!allOccupied)
                {
                    var state1 = state.ApplyAction(new GameAction((uint)Enumerable.Range(0, 7).Except(perm).First(), GameSide.SideA));
                    var expectedActions1 = Enumerable.Range(0, 7).Except(perm).Select(col => new GameAction((uint)col, GameSide.SideB));
        ...
        private IGameState fillColumn(IGameState state, int column)
        {
            state = state.ApplyAction(new GameAction((uint)column, GameSide.SideA));
            state = state.ApplyAction(new GameAction((uint)column, GameSide.SideB));
            state = state.ApplyAction(new GameAction((uint)column, GameSide.SideA));
            state = state.ApplyAction(new GameAction((uint)column, GameSide.SideB));
            state = state.ApplyAction(new GameAction((uint)column, GameSide.SideA));
            state = state.ApplyAction(new GameAction((uint)column, GameSide.SideB));
        ...
        public void IsConnectNTest()
        {
            ...
            // test vertical win condition (side A) rows = column = n first column
            state = GameStateFactory.CreateState(4, 4);
            ...
            // test vertical win condition (side A) rows = column = n last column
            state = GameStateFactory.CreateState(4, 4);
            ...
            // test horizontal win condition (side A) rows = column = n first row
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            ...
            // test diagonal win condition (side A) rows = column = n
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            state = state.ApplyAction(new GameAction(1, GameSide.SideB));
            state = state.ApplyAction(new GameAction(1, GameSide.SideA));
            ...
            // test diagonal win condition (side A, reverse) rows = columns = n
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(3, GameSide.SideA));
            state = state.ApplyAction(new GameAction(2, GameSide.SideB));
            state = state.ApplyAction(new GameAction(2, GameSide.SideA));
            ...
            // test vertical win condition (side B) rows = columns = n first column
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(3, GameSide.SideA));
            state = state.ApplyAction(new GameAction(0, GameSide.SideB));
            state = state.ApplyAction(new GameAction(1, GameSide.SideA));
            ...
            // test vertical win condition (side B) rows = columns = n last column
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            state = state.ApplyAction(new GameAction(3, GameSide.SideB));
            state = state.ApplyAction(new GameAction(1, GameSide.SideA));
            ...
            // test horizontal win condition (side B) rows = column = n
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            state = state.ApplyAction(new GameAction(0, GameSide.SideB));
            state = state.ApplyAction(new GameAction(1, GameSide.SideA));
            ...
            // test diagonal win condition (side B) rows = column = n
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(2, GameSide.SideA));
            state = state.ApplyAction(new GameAction(0, GameSide.SideB));
            state = state.ApplyAction(new GameAction(1, GameSide.SideA));
            ...
            // test diagonal win condition (side B, reverse) rows = column = n
            state = GameStateFactory.CreateState(4, 4);
            state = state.ApplyAction(new GameAction(1, GameSide.SideA));
            state = state.ApplyAction(new GameAction(3, GameSide.SideB));
            state = state.ApplyAction(new GameAction(2, GameSide.SideA));
            ...
            // test overlapping win conditions
            // test overlapping vertical win condition (Side A)
            state = GameStateFactory.CreateState(6, 7);
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            state = state.ApplyAction(new GameAction(1, GameSide.SideB));
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            ...
            // test overlapping vertical win condition (Side B)
            state = GameStateFactory.CreateState(6, 7);
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            state = state.ApplyAction(new GameAction(1, GameSide.SideB));
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            ...
            // test overlapping horizonzal win condition (Side A)
            state = GameStateFactory.CreateState(6, 7);
            state = state.ApplyAction(new GameAction(0, GameSide.SideA));
            state = state.ApplyAction(new GameAction(2, GameSide.SideB));
            state = state.ApplyAction(new GameAction(1, GameSide.SideA));
            ...
            // test overlapping diagonal win condition (Side A)
            //          A
            //      A   B
            //  A   A   B
            //  B   B   B   A
            ...
            // test overlapping diagonal win condition (Side B)
            //          B
            //      B   A
            //  B   A   B
            //  A   A   A   B
            ...
            // test overlapping revert diagonal win condition (Side A)
            //          A
            //          A   A   
            //          B   B   A
            //   B  A   A   B   B
            ...
            // test overlapping revert diagonal win condition (Side B)
            //          B
            //          A   B   
            //          B   A   B
            //      B   A   A   A
            ...
            // Special case from game engine test
            //  A   B   B   B   A
            //  A   B   A   B   A
            //  B   A   B   A   B
            //  A   B   A   B   A
        ...
        public void Base64HashTest()
        {
            ...
            Assert.Equal("BgcAAAAAAAAAAAAAAA==", hash);
